...
/* Задача 1
Создайте таблицу users_old, аналогичную таблице users. Создайте процедуру, с помощью 
которой можно переместить любого (одного) пользователя из таблицы users в таблицу users_old.
*/

-- Создание таблицы users_old, аналогичную таблице users.
DROP TABLE IF EXISTS users_old;
CREATE TABLE users_old LIKE users;
ALTER TABLE users_old ADD old_id BIGINT UNSIGNED NOT NULL;

-- Создание процедуры, для переноса любого пользователя из таблицы users в таблицу users_old.
DROP PROCEDURE IF EXISTS sp_user_move;
DELIMITER //
CREATE PROCEDURE sp_user_move(
user_id BIGINT UNSIGNED, OUT  tran_result varchar(100))
BEGIN	
    ...
    START TRANSACTION;
    INSERT INTO users_old (firstname, lastname, email, old_id)
...
-- вызов процедуры, для переноса пользователя c id = @user_move_id из users в users_old.
SET @user_move_id = 5;
CALL sp_user_move(@user_move_id, @tran_result); 
SELECT @tran_result;
...
-- Создание функции, которая будет возвращать приветствие, в зависимости от текущего времени суток.
...
BEGIN
    ...
    SET @time_current = CURRENT_TIME(), @error_time = 'Неизвестное время суток',
        @time_night_start = '00:00:00', @time_night_end = '05:59:59', @say_night = 'Доброй ночи',
        @time_morning_start = '06:00:00', @time_morning_end = '11:59:59', @say_morning = 'Доброе утро', 
        @time_day_start = '12:00:00', @time_day_end = '17:59:59', @say_day = 'Добрый день', 
        @time_evening_start = '18:00:00', @time_evening_end = '23:59:59', @say_evening = 'Добрый вечер'; 
    SET request = (
    SELECT 			
        CASE
            WHEN @time_current BETWEEN @time_night_start AND @time_night_end THEN @say_night
            WHEN @time_current BETWEEN @time_morning_start AND @time_morning_end THEN @say_morning
            WHEN @time_current BETWEEN @time_day_start AND @time_day_end THEN @say_day
            WHEN @time_current BETWEEN @time_evening_start AND @time_evening_end THEN @say_evening
            ELSE @error_time
        END AS time_of_day);             
    RETURN request;
END//
...
-- Создание триггера архивации данных при создании в users.
DROP TRIGGER if exists create_user_archive_to_logs;
DELIMITER //
CREATE TRIGGER create_user_archive_to_logs AFTER INSERT ON users
FOR EACH ROW
BEGIN
    SET @name_table = 'users';
    INSERT INTO `logs` SET name_table = @name_table, id_table = NEW.id;
END//
DELIMITER ;

-- Создание триггера архивации данных при создании в communities.
DROP TRIGGER if exists create_communities_archive_to_logs;
DELIMITER //
CREATE TRIGGER create_communities_archive_to_logs AFTER INSERT ON `communities`
FOR EACH ROW
BEGIN
    SET @name_table = 'communities';
    INSERT INTO `logs` SET name_table = @name_table, id_table = NEW.id;
END//
DELIMITER ;

-- Создание триггера архивации данных при создании в messages.
DROP TRIGGER if exists create_messages_archive_to_logs;
DELIMITER //
CREATE TRIGGER create_messages_archive_to_logs AFTER INSERT ON messages
FOR EACH ROW
BEGIN
    SET @name_table = 'messages';
    INSERT INTO `logs` SET name_table = @name_table, id_table = NEW.id;
END//
DELIMITER ;

-- Проверка триггера таблицы users.
SET @firstname = 'Ren', @lastname = 'New', @email = 'asd@dsa.org';
INSERT INTO users (firstname, lastname, email) VALUES 
    (@firstname, @lastname, @email);

-- Проверка триггера таблицы communities.
SET @new_communitie = 'west';
INSERT INTO `communities` (`name`) VALUES (@new_communitie);

-- Проверка триггера таблицы messages.
SET @from_user_id = 1, @to_user_id = 10, @message = 'Hello';
INSERT INTO messages (from_user_id, to_user_id, body) VALUES (@from_user_id, @to_user_id, @message);

-- Вывод таблицы logs.
SELECT id_table, name_table, created_at FROM `logs`;